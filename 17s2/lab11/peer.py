...
# show other's assessment
@app.route('/assessments', methods=['POST','GET'])
def assessments():
	if 'zid' not in session:
		return render_template('peer_login.html')
	students = sorted(os.listdir('assessments'))
	#print(students)
	dic = {}
	lis = []
	#szid = zid
	szid = session['zid']
	print("  -- session = {}".format(szid))
	for f in students:
		print(f)
		x = f.split('.')
		print("current x = ",x,end="")
		print("  -- session = {}".format(szid))
		
		if x[1] == szid:
			with open("assessments/" + f) as file:
				line = file.readline()
				lis.append(line)
				dic.update({x[0]:line})
	print(dic)
	
	return render_template('peer_assessments.html',grade_dic=dic,leng=len(lis),lis=lis)
