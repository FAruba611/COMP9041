...
sub main() {
	...
	my $dir = "/import/ravel/3/z5137858/public_html/lab10/accounts/";
	my @file_name_dep_1;
	opendir (DIR,$dir) or die;
	#@file_name_dep_1 = readdir DIR;
	@file_name_dep_1 = `find $dir -type f`;
	
	$tag = 0;
	foreach $file_name (@file_name_dep_1) {
		...
		if ($type eq "name") {
			...
			print "user = $username\n";
			print "getn = $actual_name[0]\n";
			if ($actual_name[0] eq $username) {
				$tag = 1;
				next;
			}
		}
		if ($type eq "password" && $tag == 1) {
			open (INFO, "$file_name");
			while(<INFO>){
				chomp $_;
				$_ =~ s/\ //g;
